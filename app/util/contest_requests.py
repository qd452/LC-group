...
def search_pagination(page, contest_num, users):
    r = requests.get(
        f'https://leetcode.com/contest/api/ranking/{contest_num}/?pagination={page}&region=global').json()
...
def get_user_info_parallel(users):
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        future_to_user = {executor.submit(get_user_info, user): user for user in users}
...
def get_user_info(user):
    ...
    solved_questions_obj = next(x for x in list_group_item if 'Solved Question' in x.text)
    rating = rating_obj.find('span').text.strip()
    solved_questions = solved_questions_obj.find('span').text.strip().split('/')[0].strip()
    return rating, solved_questions


def format_data(users, questions_dct, start_time):
    ...
    for u in users:
        ...
        for q_title, q_id in questions_dct.items():
            ...
            try:
                ...
                contents = f'{q_time}({fail_count})' if fail_count > 0 else str(q_time)
            except:
                pass
...
def get_contest(contest_num, users):
    rslt = []
    ...
    submitted_user_num = contest['user_num']
    print(title)
    ...
    # print(questions_dct)
    r = requests.get(f'https://leetcode.com/contest/api/ranking/{contest_num}/?pagination=2&region=global')
    pagination = r.json()
    registered_user_num = pagination['user_num']
    total_pages = registered_user_num // 25

    with ThreadPoolExecutor(max_workers=50) as executor:
        futures = [executor.submit(lambda x: search_pagination(*x), (page, contest_num, users)) for page in range(1, total_pages + 1)]
        for future in as_completed(futures):
            rslt.extend(future.result())

    # with open(f'{contest_num}.json', 'w') as f:
    #     json.dump(rslt, f, indent=4)
    df = format_data(rslt, questions_dct, start_time)
    print(df)
...
if __name__ == '__main__':
    ...
    users = {'qd452', 'guoqiang2648', 'thread_start', 'liu_diansheng', 'sylyongli', 'lijiang3800045', 'jessica_x_1028',
             'lqianzhen', 'coffeebenzene', 'gohuishan', 'M00nwell', 'pumbaa5', 'farmerboy'}
    get_contest(contest_num, users)
